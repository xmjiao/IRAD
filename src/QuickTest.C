...
namespace IRAD {
  ...
  /// \addtogroup irad_group
  /// @{
 
  ///
  /// \brief ComLineObject for testing app
  ///
  class TestComLine : public Util::ComLineObject
  {
  public:
    TestComLine()
      : Util::ComLineObject()
  ...
  int MyTestProgram(int argc,char *argv[])
  {
    ...
    MyParams params;
    //  MyParams::iterator pi = params.begin();
    //  while(pi != params.end())
...
{
  return(IRAD::MyTestProgram(argc,argv));
}
